...
async function getCategoryBrandEntries(baseUrl) {
  const categories = await Category.find(
    ...
    { category_slug: 1, md5_cat_name: 1, updatedAt: 1 }
  ).lean();
  ...
  for (const category of categories) {
    ...
    for (const brand of brands) {
      const key = `${category.category_slug}::${brand.brand_slug}`;
      if (seen.has(key)) continue;
      seen.add(key);

      entries.push(
        buildUrlEntry({
          loc: `${baseUrl}/category/brand/${category.category_slug}/${brand.brand_slug}`,
          lastmod: brand.updatedAt || category.updatedAt,
          changefreq: "weekly",
          priority: "0.7",
        })
      );
